...
# Optional imports for standalone script usage
try:
    from base_img_s3 import upload_base64_image_to_s3
except ImportError:
    upload_base64_image_to_s3 = None
...
def parse_questions_from_content(content: str) -> List[Dict[str, Any]]:
    """Parse questions from markdown content string.
...
def parse_questions_from_file(file_path: Path) -> List[Dict[str, Any]]:
    ...
    with open(file_path, 'r', encoding='utf-8-sig') as f:
        content = f.read()
    return parse_questions_from_content(content)


def parse_md_to_csv(md_content: str, output_path: Optional[Path] = None) -> str:
    """Parse markdown content and return CSV string.
    ...
    """
    questions = parse_questions_from_content(md_content)
    csv_content = write_questions_to_csv_string(questions)
...
def write_questions_to_csv_string(questions: List[Dict[str, Any]]) -> str:
    """Write questions to CSV string.
